...
Nrf24L01_buf[0] = 1;   // 有效数据长度 = 1
Nrf24L01_buf[1] = '1'; // 表示一次中断事件
...
while(1)
{
    u32 now_counter = Get_Couter();

    OLED_ShowNum(1,1,now_counter,4);
    ...
    if(now_counter != last_counter)
{
    ...
    Nrf24L01_buf[0] = strlen(tx_str);

    for(i = 0; i < Nrf24L01_buf[0]; i++)
    {
        Nrf24L01_buf[i + 1] = tx_str[i];
    }
